...
Current path-traced output at 64 samples per pixel, without the HUD:

| Luna | Eclipse on distant Vantus |
| --- | --- |
| ![Path-traced crescent Moon](docs/pathtraced-moon.png) | ![Iri and its shadow on Vantus](docs/pathtraced-vantus-eclipse.png) |

These are generated by the `gpu_scene_smoke` test below. The original project
...
light, distant-surface self-intersection, deep-zoom label alignment, resize,
presentation, and both built-in scenes (including the Vantus eclipse preset).
